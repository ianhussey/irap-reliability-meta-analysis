...
# fit random Effects model 
fit_oddeven_sensitivity <- data_D_scores_internal_consistency_oddeventrials %>%
  filter(!str_detect(domain, "Sexuality")) %>%
  filter(!domain %in% c("Gender stereotypes (3)")) %>%
  rma(yi   = yi, 
      vi   = vi, 
...
# make predictions 
predictions_oddeven <-
  predict(fit_oddeven_sensitivity, digits = 5) %>%
  as.data.frame() %>%
  round_df(2)

# plot
metafor::forest(fit_oddeven_sensitivity,
                xlab = "Spearman-Brown correlation",
                addcred = TRUE,
                refline = FALSE,
                transf = transf.iabt,
                xlim = c(-1.4, 1.6),
                at = c(0, 0.25, 0.5, 0.75, 1),
                mlab = add_heterogeneity_metrics_to_forest(fit_oddeven_sensitivity))
text(-1.4, 43, "Study", pos = 4)
text(1.6, 43, "Spearman-Brown correlation [95% CI]", pos = 2)

# summarize results
meta_effect <- 
  paste0("Meta analysis: k = ", fit_oddeven_sensitivity$k, 
         ", r_sb = ",  janitor::round_half_up(transf.iabt(predictions_oddeven$pred), 2), 
         ", 95% CI [", janitor::round_half_up(transf.iabt(predictions_oddeven$ci.lb), 2), ", ", 
         janitor::round_half_up(transf.iabt(predictions_oddeven$ci.ub), 2), "]", 
         ", 95% CR [", janitor::round_half_up(transf.iabt(predictions_oddeven$pi.lb), 2), ", ", 
...
meta_heterogeneity <- 
  paste0("Heterogeneity tests: Q(df = ", fit_oddeven_sensitivity$k - 1, ") = ", janitor::round_half_up(fit_oddeven_sensitivity$QE, 2), 
         ", p ", ifelse(fit_oddeven_sensitivity$QEp < 0.0001, "< .0001", paste0("= ", as.character(janitor::round_half_up(fit_oddeven_sensitivity$QEp, 4)))),
         ", tau^2 = ", janitor::round_half_up(fit_oddeven_sensitivity$tau2, 2), 
         ", I^2 = ",   janitor::round_half_up(fit_oddeven_sensitivity$I2, 2),
         "%, H^2 = ",   janitor::round_half_up(fit_oddeven_sensitivity$H2, 2))
...
# fit random Effects model 
fit_firstsecond_sensitivity <- data_D_scores_internal_consistency_firstsecondhalves %>%
  filter(!str_detect(domain, "Sexuality")) %>%
  filter(!domain %in% c("Gender stereotypes (3)")) %>%
  rma(yi   = yi, 
      vi   = vi, 
...
# make predictions 
predictions_firstsecond <-
  predict(fit_firstsecond_sensitivity, digits = 5) %>%
  as.data.frame() %>%
  round_df(2)

# plot
metafor::forest(fit_firstsecond_sensitivity,
                xlab = "Spearman-Brown correlation",
                addcred = TRUE,
                refline = FALSE,
                transf = transf.iabt,
                xlim = c(-1.4, 1.6),
                at = c(0, 0.25, 0.5, 0.75, 1),
                mlab = add_heterogeneity_metrics_to_forest(fit_firstsecond_sensitivity))
text(-1.4, 43, "Study", pos = 4)
text(1.6, 43, "Spearman-Brown correlation [95% CI]", pos = 2)

# summarize results
meta_effect <- 
  paste0("Meta analysis: k = ", fit_firstsecond_sensitivity$k, 
         ", r_sb = ",  janitor::round_half_up(transf.iabt(predictions_firstsecond$pred), 2), 
         ", 95% CI [", janitor::round_half_up(transf.iabt(predictions_firstsecond$ci.lb), 2), ", ", 
         janitor::round_half_up(transf.iabt(predictions_firstsecond$ci.ub), 2), "]", 
         ", 95% CR [", janitor::round_half_up(transf.iabt(predictions_firstsecond$pi.lb), 2), ", ", 
...
meta_heterogeneity <- 
  paste0("Heterogeneity tests: Q(df = ", fit_firstsecond_sensitivity$k - 1, ") = ", janitor::round_half_up(fit_firstsecond_sensitivity$QE, 2), 
         ", p ", ifelse(fit_firstsecond_sensitivity$QEp < 0.0001, "< .0001", paste0("= ", as.character(janitor::round_half_up(fit_firstsecond_sensitivity$QEp, 4)))),
         ", tau^2 = ", janitor::round_half_up(fit_firstsecond_sensitivity$tau2, 2), 
         ", I^2 = ",   janitor::round_half_up(fit_firstsecond_sensitivity$I2, 2),
         "%, H^2 = ",   janitor::round_half_up(fit_firstsecond_sensitivity$H2, 2))
...
average_mean_and_sd_overall <- data_D_scores |>
  group_by(domain) |>
  summarize(mean = mean(D_overall),
            sd = sd(D_overall),
            .groups = "drop") |>
...
average_mean_and_sd_overall

percentiles <- data_D_scores |>
  summarize(D_percentile_0.025 = quantile(D_overall, probs = 0.025),
            D_percentile_0.975 = quantile(D_overall, probs = 0.975)) |> 
  round_df(2)
...
sem_estimate <- sem(sd = average_mean_and_sd_overall |> pull(weighted_mean_sd), 
...
The precision of an individual's score is their overall *D* score ± `r individual_95ci_half_width`.

The weighted mean *D* score across all domains is `r average_mean_and_sd_overall |> pull(weighted_mean_mean)`, i.e., a slightly positive effect (slightly faster on the consistent blocks than the inconsistent blocks). 

However, due to the task's poor reliability, an individual who demonstrates an (overall) *D* score = `r average_mean_and_sd_overall |> pull(weighted_mean_mean)` can actually only be said to have a score within the interval *D* = 95% [`r average_mean_and_sd_overall |> pull(weighted_mean_mean) - individual_95ci_half_width`, `r average_mean_and_sd_overall |> pull(weighted_mean_mean) + individual_95ci_half_width`], i.e. anywhere between much faster on the consistent blocks and much faster on the inconsistent blocks. 

Combine this with the fact that 95% of all observed overall *D* scores in the dataset lie within the range = `r as.numeric(percentiles$D_percentile_0.025)` to `r as.numeric(percentiles$D_percentile_0.975)`. This means that estimation of the average participant's score covers most of the range (`r janitor::round_half_up(individual_95ci_half_width*2 / (as.numeric(percentiles$D_percentile_0.975) - as.numeric(percentiles$D_percentile_0.025)) * 100, 2)`%) of all observed participants. i.e., the average individual participant cannot be distinguished from the *D* scores demonstrated by the **great majority** of other participants, from extremely positive to extremely negative.
...
fit_internal_consistency_response_option_locations <- 
  ...
  filter(!str_detect(domain, "Sexuality")) %>%
  filter(!domain %in% c("Gender stereotypes (3)")) %>%
  filter(!is.na(response_option_locations)) %>%
  rma(yi   = yi, 
      vi   = vi, 
