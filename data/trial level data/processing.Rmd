...
data_input <- read_csv("raw data/test-retest IRAPs/raw/data sexuality test retest - trial level.csv")
...
data_input <- read_csv("raw data/nonevaluative IRAPs/raw/IRAP 2012 VB format files/data disgust - trial level.csv") 
...
data_evalautive_iraps <- read_csv("raw data/evaluative IRAPs/data_cleaned_trial_level.csv") %>%
  filter(timepoint %in% c(1,2)) %>%
  mutate(timepoint = recode(timepoint,
                            "1" = "baseline",
                            "2" = "followup"))

# filter participants who received an intervention before their second IRAP, these aren't strictly test-retest.
participants_to_be_excluded <- read_csv("raw data/evaluative IRAPs/component studies/participants who received faking instructions.csv") %>%
  pull(participant_id)

data_evalautive_iraps_after_intervention_exclusions <- data_evalautive_iraps %>%
  filter(!participant %in% participants_to_be_excluded)

# filter participants who had both baseline and followup data
temp <- data_evalautive_iraps_after_intervention_exclusions %>%
  ungroup() %>%
...
data_input_evaluative <- 
  read.csv("raw data/evaluative IRAPs/data_cleaned_trial_level.csv") %>%
...
data_cleaned_ic_nonevaluative <- 
  read.csv("raw data/nonevaluative IRAPs/data_cleaned_trial_level.csv") %>%
  mutate(trial_odd_even = ifelse(row_number() %% 2 == 1, "D_odd", "D_even")) %>%
  # tidy domain
  rename(domain = experiment) %>%
  mutate(domain = dplyr::recode(domain,
