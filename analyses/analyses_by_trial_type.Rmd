...
# fit random Effects model 
fit_internal_consistency_permuted_estimates_native <- 
  rma.mv(yi     = alpha, 
         V      = se^2,
         random = ~ 1 | domain,
         data   = data_ic_permuted,
...
# make predictions 
predictions_permutations_native <-
  predict(fit_internal_consistency_permuted_estimates_native, digits = 5) %>%
  as.data.frame() %>%
  round_df(2)

# plot
metafor::forest(data_ic_permuted$alpha, 
                data_ic_permuted$se^2,
                ...
                cex.lab = 1, cex.axis = 1, # increase size of x-axis title/labels
                lty = c("solid", "blank")) # remove horizontal line at top of plot
points(sort(data_ic_permuted$alpha), nrow(data_ic_permuted):1, pch = 19, cex = 0.5) # draw points one more time to make them easier to see
addpoly(fit_internal_consistency_permuted_estimates_native, mlab = "", cex = 1, addcred = TRUE) # add summary polygon at bottom and text
#text(-1, -1, "RE Model", pos = 4, offset = 0, cex = 1)

# summarize results
meta_effect <- 
  paste0("Meta analysis: k = ", fit_internal_consistency_permuted_estimates_native$k, 
         ", alpha = ",  janitor::round_half_up(predictions_permutations_native$pred, 2), 
         ", 95% CI [", janitor::round_half_up(predictions_permutations_native$ci.lb, 2), ", ", 
         janitor::round_half_up(predictions_permutations_native$ci.ub, 2), "]", 
         ", 95% CR [", janitor::round_half_up(predictions_permutations_native$pi.lb, 2), ", ", 
         janitor::round_half_up(predictions_permutations_native$pi.ub, 2), "]") 

# save to disk for making pdf plot
#write_rds(fit_internal_consistency_permuted_estimates_native, "models/fit_internal_consistency_permuted_estimates_native.rds")
...
TODO: polygon for caterpillar plot
