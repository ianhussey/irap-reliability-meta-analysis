...
- lincoln-hitler and friend enemy iraps are currently filtered out. Chad says some participants got faking instructions and other didn't; these need to be excluded
- standardize domain names between IC and TRT in data processing
- ensure ICC calculations are correct - some domains look suspiciously symmetrical around zero. plotting of D scores needed.
...
data_test_retest_icc <- 
  ...
  # FILTER THIS DOMAIN UNTIL CHAD GIVES ME DATA ON WHICH PARTICIPANTS WERE INSTRUCTED TO FAKE
  filter(!domain %in% c("Friend-enemy", "Lincoln-Hitler"))
...
data_internal_consistency_oddeventrials <- 
  ...
  # FILTER THIS DOMAIN UNTIL CHAD GIVES ME DATA ON WHICH PARTICIPANTS WERE INSTRUCTED TO FAKE
  filter(!domain %in% c("Friend-Enemy evaluations", "Lincoln-Hitler evaluations"))
...
meta_effect <- 
  paste0("Meta analysis: k = ", fit_oddeven$k, 
         ", r_sb = ",  round(transf.ztor(predictions_oddeven$pred), 2), 
         ", 95% CI [", round(transf.ztor(predictions_oddeven$ci.lb), 2), ", ", 
         round(transf.ztor(predictions_oddeven$ci.ub), 2), "]", 
         ", 95% CR [", round(transf.ztor(predictions_oddeven$cr.lb), 2), ", ", 
         round(transf.ztor(predictions_oddeven$cr.ub), 2), "]") 
...
meta_effect <- 
  paste0("Meta analysis: k = ", fit_firstsecond$k, 
         ", r_sb = ",  round(transf.ztor(predictions_firstsecond$pred), 2), 
         ", 95% CI [", round(transf.ztor(predictions_firstsecond$ci.lb), 2), ", ", 
         round(transf.ztor(predictions_firstsecond$ci.ub), 2), "]", 
         ", 95% CR [", round(transf.ztor(predictions_firstsecond$cr.lb), 2), ", ", 
         round(transf.ztor(predictions_firstsecond$cr.ub), 2), "]") 
...
meta_effect <- 
  paste0("Meta analysis: k = ", fit_internal_consistency_permuted_estimates$k, 
         ", alpha = ",  round(transf.ztor(predictions_permutations$pred), 2), 
         ", 95% CI [", round(transf.ztor(predictions_permutations$ci.lb), 2), ", ", 
         round(transf.ztor(predictions_permutations$ci.ub), 2), "]", 
         ", 95% CR [", round(transf.ztor(predictions_permutations$cr.lb), 2), ", ", 
         round(transf.ztor(predictions_permutations$cr.ub), 2), "]") 
...
text(-1.5, 8, "Study", pos = 4)
text(1.7, 8, "ICC [95% CI]", pos = 2)
...
max_cor <- observed_r(true_r = 1.0, reliability_a = transf.iabt(predictions_permutations$pred), reliability_b = 1.0)
...
current_ic <- transf.iabt(predictions_permutations$pred)
...
Using the absolute agreement estimate of 0.40, the IRAP would have to be `r trt_length_increase_.70` times longer for it to have a test-retest of >=.70, `r trt_length_increase_.80` times longer for it to have a test-retest of >=.80, or `r trt_length_increase_.90` times longer for it to have a test-retest of >=.90.
