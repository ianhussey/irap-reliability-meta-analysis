...
subtitle: "Published vs unpublished studies"
author: "Ian Hussey"
...
data_parameters <- 
  ...
  clean_names() |>
  mutate(data_published_binary = ifelse(data_published_abbreviated == "NA", FALSE, TRUE)) |>
  select(domain, data_published_binary)
...
data_D_scores_internal_consistency_permuted_estimates <- 
  ...
  left_join(data_parameters, by = "domain") |>
  mutate(data_published_binary = fct_relevel(as.factor(data_published_binary), "TRUE", "FALSE"))
# |>
#   filter(data_published_binary == TRUE)
...
data_D_scores_internal_consistency_permuted_estimates %>%
  group_by(data_published_binary) |>
...
meta_effect_ic <-
  ...
  mutate(alpha = ifelse(published == FALSE, alpha + fit_ic$beta[1], alpha),
         ci_lower = ifelse(published == FALSE, ci_lower + fit_ic$beta[1], ci_lower),
         ci_upper = ifelse(published == FALSE, ci_upper + fit_ic$beta[1], ci_upper)) |>
