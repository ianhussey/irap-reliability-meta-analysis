...
data_input <- read_csv("tidied data/test-retest IRAPs/component studies/data sexuality test retest - trial level.csv")
...
data_input <- read_csv("tidied data/nonevaluative IRAPs/component studies/IRAP 2012 VB format files/data disgust - trial level.csv") 
...
data_rts_and_summmaries_disgust <- data_input %>%
...
data_testretest_cleaned_trial_level <- 
  bind_rows(data_rts_and_summmaries_sexuality, 
            data_rts_and_summmaries_disgust,
...
data_participants_with_correct_n_trials <- data_input_evaluative %>%
  count(unique_id, timepoint) %>%
  filter(n %in% c(144, 192, 288, 432)) # methodologically plausible number of trials
...
data_n_trials <- data_combined_ic %>%
  group_by(unique_id, timepoint) %>%
  summarise(n = n()) %>%
  filter(n %in% c(144, 192, 240, 288, 432))
