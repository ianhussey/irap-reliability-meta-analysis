...
plot(fit_gosh_ic, breaks = 20)

tibble(mean_es_est = round(mean(fit_gosh_ic$res$estimate, na.rm = TRUE), 2),
       mean_I2_est = round(mean(fit_gosh_ic$res$I2, na.rm = TRUE), 1),
       mean_H2_est = round(mean(fit_gosh_ic$res$H2, na.rm = TRUE), 1)) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
...
if(!file.exists("models/fit_gosh_trt.rds")){
  ...
  fit_gosh_trt <- gosh(fit_internal_consistency_permuted_estimates, 
                       parallel = "multicore", 
                       ncpus = 4)
...
plot(fit_gosh_trt, breaks = 20)

tibble(mean_es_est = round(mean(fit_gosh_trt$res$estimate, na.rm = TRUE), 2),
       mean_I2_est = round(mean(fit_gosh_trt$res$I2, na.rm = TRUE), 1),
       mean_H2_est = round(mean(fit_gosh_trt$res$H2, na.rm = TRUE), 1)) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
...
max_r_observed = r_true \* sqrt(r_xx*r_yy)
...
medium_cor <- observed_r(true_r = 0.5, reliability_a = transf.iabt(predictions_permutations$pred), reliability_b = 1.0)

```

Given this meta estimate of reliability, assuming that the internal consistency of an external variable is perfect (1.0), the maximum correlation between the IRAP and that external variable is `r max_cor`. For a true correlation of medium size (r = .5), the observed correlation would be `r medium_cor`.
